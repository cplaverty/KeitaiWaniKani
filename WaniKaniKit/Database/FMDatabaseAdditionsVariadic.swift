...
extension FMDatabase {
    ...
    private func valueForQuery<T>(_ sql: String, values: [AnyObject]?, completionHandler: (FMResultSet) -> T!) throws -> T! {
        var result: T!
        
        let rs = try executeQuery(sql, values: values)
        defer { rs.close() }
        if rs.next() {
            let obj: AnyObject! = rs.object(forColumnIndex: 0)
            if !(obj is NSNull) {
    ...
    func stringForQuery(_ sql: String, _ values: AnyObject...) throws -> String! {
    ...
    func intForQuery(_ sql: String, _ values: AnyObject...) throws -> Int32! {
    ...
    func longForQuery(_ sql: String, _ values: AnyObject...) throws -> Int! {
    ...
    func boolForQuery(_ sql: String, _ values: AnyObject...) throws -> Bool! {
    ...
    func doubleForQuery(_ sql: String, _ values: AnyObject...) throws -> Double! {
    ...
    func dateForQuery(_ sql: String, _ values: AnyObject...) throws -> Date! {
    ...
    func dataForQuery(_ sql: String, _ values: AnyObject...) throws -> Data! {

 ...
 *	=== Changelog ===
 *
 *  1.1.3 (31 August 2015)
 *  - Modified to work in iOS WKWebView
 *
 *  1.1.2 (11 March 2014)
...
function bindHotkey()
{
    jQuery(document).on('keypress.reviewScreen', function (event) {
        if ($('#reviews').is(':visible') && !$('*:focus').is('textarea, input')) {
            ...
            case 176: //Firefox '~'
            case 192: //Chrome '~'
                scriptLog('Ignore answer triggered by key code ' + event.keyCode)
...
function logError(error) {
    window.webkit.messageHandlers.debuglog.postMessage({ 'error': error.name, 'message': error.message, 'stack': error.stack });
}
...
try {
    if (window.location.hostname.endsWith('wanikani.com') && window.location.pathname.startsWith('/review/session')) {
        scriptInit();
    }
}
catch (err) {
